...
def main():
    ...
    ap.add_argument("--no-reset-on-recovery", action="store_true",
                    help="BACKUP/HOMING → TASK 转换时不强制 go_home_to_reset_pose，让 supervisor "
                         ...
                         "wipe 是连续涂抹任务，理论上 mid-episode resume 比 pickup 更可行（无需"
                         "重新抓物块），但实测仍不推荐：BACKUP 期间海绵位置 / 工作面状态可能已变。")
    ap.add_argument("--max-episode-steps", type=int, default=400,
                    help="TASK 模式单 episode 最大步数（默认 400 = 40s @ 10Hz；wipe 较长）。"
    ...
    # ---------- Wipe task config（拿 reset_pose / 阻抗参数 / 工作空间边界给 TASK 用） ----------
    bias_monitor_save_path = None
    if args.bias_monitor and args.bias:
        from datetime import datetime
        Path("charts").mkdir(exist_ok=True)
        ts = datetime.now().strftime("%Y%m%d_%H%M%S")
        bias_monitor_save_path = f"charts/bias_deploy_wipe_with_backup_{ts}"
        print(f"[OK] BiasMonitor enabled → {bias_monitor_save_path}.{{npz,png}}")
    task_cfg = make_task_config(
        task="wipe", use_bias=args.bias, reward_backend="keyboard",
    ...
    # 启动 homing 完成 + 操作员按 S → 第一个 episode 采新 bias
    if bias_ctrl is not None:
        ...
        while not aborted():
            ...
            if (prev_mode in (Mode.BACKUP, Mode.HOMING)) and (new_mode == Mode.TASK):
                if args.no_reset_on_recovery:
                    ...
                    # wipe 的 gripper_locked="closed"：recovery 期间 go_home 内部仍调
                    # /open_gripper 把海绵放掉（操作员要重新摆好海绵起点），因此 GripperCommander
                    # 状态需要同步成 "open"。下一集 BC 会继续输出 close 信号自然合上。
                    print(f"\n[recover] {prev_mode.name}→TASK：force home + 释放海绵 + 等 S 继续")
                    homing_ok = True
                    if not args.dry_run:
                    ...
                    if homing_ok:
                        ...
                        print("=== [recover] 完成。摆好海绵 + 收手离开 + 按 S 继续 ===")
                        keyboard_listener.wait_for_start()
                        if bias_ctrl is not None:
            ...
            # 注：这里只 poll outcome；wait_for_start (S 键) 在每个 episode 开头处理。
            ...
            if episode_done:
                homing_ok = True
                if not args.dry_run:
                    # 新集边界：clear+anchor → home → resample=True（采新 bias）
                    if bias_ctrl is not None:
                        bias_ctrl.begin_transition()
                    # gripper_locked="closed" wipe → 不 force_open（海绵继续夹着）
                    homing_ok = go_home_to_reset_pose(
                        reset_pose_6d, precision_param, compliance_param,
                        hand_detector, cam_mgr, d_safe=d_safe,
                    )
                ...
                if homing_ok:
                    ...
                    keyboard_listener.mark_episode_ended()
                    # 等下次操作员按 S 才进下一集
                    print(f"=== ep done. 摆好海绵起点 + 收手离开工作区 + 按 S 开始下一集 ===")
                    keyboard_listener.wait_for_start()
                    if bias_ctrl is not None:
                        bias_ctrl.finish_transition(
                            ep_num=success_count + fail_count,
