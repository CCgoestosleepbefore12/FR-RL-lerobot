...
and any future server-side changes live in a single repo alongside the training code.
"""
...
from serl_franka_controllers.msg import ZeroJacobian, BiasedState
...
class FrankaServer:
    ...
    def __init__(self, robot_ip, gripper_type, ros_pkg_name, reset_joint_target):
        ...
        # Encoder bias state (fault injection point D).
        # self.q / self.pos stay as true values from franka_state (used by joint reset).
        # self.q_biased / self.pos_biased are populated from the controller's
        # biased_state topic and returned by /getstate.
        self.current_bias = np.zeros(7)
        self.q_biased = np.zeros(7)
        self.pos_biased = np.zeros(7)
        self._biased_state_received = False
        ...
        self.bias_pub = rospy.Publisher(
            "/encoder_bias", Float64MultiArray, queue_size=1, latch=True
        )
        ...
        self.biased_state_sub = rospy.Subscriber(
            "/cartesian_impedance_controller/biased_state",
            BiasedState,
            self._set_biased_state,
        )
        time.sleep(1)
        ...
        # Publish zero bias once to clear any leftover latched value.
        self.publish_encoder_bias(np.zeros(7))

    def publish_encoder_bias(self, bias):
        """Publish encoder bias to /encoder_bias (latched) and cache locally."""
        bias_arr = np.asarray(bias, dtype=np.float64).reshape(-1)
        if bias_arr.size != 7:
            raise ValueError(f"encoder bias must have 7 elements, got {bias_arr.size}")
        msg = Float64MultiArray()
        msg.data = bias_arr.tolist()
        self.bias_pub.publish(msg)
        self.current_bias = bias_arr
    ...
    def _set_currpos(self, msg):
        ...
        # Fallback: until biased_state topic arrives, mirror true values.
        if not self._biased_state_received:
            self.q_biased = self.q.copy()
            self.pos_biased = self.pos.copy()
        try:
    ...
    def _set_biased_state(self, msg):
        """Cache biased q and pose published by the C++ impedance controller.

        When bias is zero, these equal the true values; otherwise they reflect
        FK(q_true + bias) so /getstate returns a consistent biased observation.
        """
        q_biased = np.array(list(msg.q_biased)).reshape((7,))
        tmatrix = np.array(list(msg.O_T_EE_biased)).reshape(4, 4).T
        r = R.from_matrix(tmatrix[:3, :3])
        pos_biased = np.concatenate([tmatrix[:3, -1], r.as_quat()])
        self.q_biased = q_biased
        self.pos_biased = pos_biased
        self._biased_state_received = True
...
def main(_):
    ...
    # pose/q are the BIASED values published by the controller's biased_state
    # topic (equal to true values when bias is zero). dq/force/torque/jacobian
    # stay as is; see fault_injection_architecture.md for the rationale.
    @webapp.route("/getstate", methods=["POST"])
    def get_state():
        return jsonify(
            {
                "pose": np.array(robot_server.pos_biased).tolist(),
                "vel": np.array(robot_server.vel).tolist(),
                "force": np.array(robot_server.force).tolist(),
                "torque": np.array(robot_server.torque).tolist(),
                "q": np.array(robot_server.q_biased).tolist(),
    ...
    # Route for setting encoder bias (fault injection)
    @webapp.route("/set_encoder_bias", methods=["POST"])
    def set_encoder_bias():
        data = request.json or {}
        bias = data.get("bias")
        if bias is None or len(bias) != 7:
            return jsonify({"error": "bias must be a list of 7 floats"}), 400
        try:
            robot_server.publish_encoder_bias(bias)
        except ValueError as e:
            return jsonify({"error": str(e)}), 400
        return jsonify({"bias": robot_server.current_bias.tolist()})

    @webapp.route("/clear_encoder_bias", methods=["POST"])
    def clear_encoder_bias():
        robot_server.publish_encoder_bias(np.zeros(7))
        return jsonify({"bias": robot_server.current_bias.tolist()})

    @webapp.route("/get_encoder_bias", methods=["POST"])
