...
class AddTeleopActionAsComplimentaryDataStep(ComplementaryDataProcessorStep):
    ...
    def complementary_data(self, complementary_data: dict) -> dict:
        ...
        teleop_action = self.teleop_device.get_action()
        new_complementary_data[TELEOP_ACTION_KEY] = teleop_action
...
class AddTeleopEventsAsInfoStep(InfoProcessorStep):
    ...
    def info(self, info: dict) -> dict:
        ...
        # Convert Enum keys to strings to avoid pickling issues across processes
        teleop_events_str = {
            k.value if isinstance(k, TeleopEvents) else k: v 
            for k, v in teleop_events.items()
        }
        new_info.update(teleop_events_str)
        return new_info
...
class InterventionActionProcessorStep(ProcessorStep):
    ...
    def __call__(self, transition: EnvTransition) -> EnvTransition:
        ...
        # Override action ONLY if intervention is active
        # In online learning mode, we want to use policy actions when not intervening
        has_teleop = teleop_action is not None and (not isinstance(teleop_action, (dict, list)) or len(teleop_action) > 0)
        if is_intervention and has_teleop:
            if isinstance(teleop_action, dict):
                # Teleop返回 {delta_x, delta_y, delta_z, gripper}
                # 需要补全为和策略输出一致的维度: [dx, dy, dz, rx, ry, rz, gripper]
                action_list = [
                    teleop_action.get("delta_x", 0.0),
                    teleop_action.get("delta_y", 0.0),
                    teleop_action.get("delta_z", 0.0),
                ]
                # 补零: rotation维度 (rx, ry, rz)
                # 计算需要填充多少维: 策略action总维度 - 3(位置) - 1(gripper) = rotation维度数
                policy_action_dim = action.shape[-1] if action.dim() > 0 else 0
                n_rotation = max(0, policy_action_dim - 3 - (1 if self.use_gripper else 0))
                action_list.extend([0.0] * n_rotation)
                if self.use_gripper:
        ...
        # 只在人类标记成功时覆盖reward，否则保留env reward（仿真环境有物理reward）
        if success:
        ...
        # Update complementary data with the action that was actually executed
        # This ensures the recorded action matches what was executed in the environment
