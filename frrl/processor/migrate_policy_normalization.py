...
import draccus
from frrl.configs.types import FeatureType, NormalizationMode, PolicyFeature
from frrl.configs.policies import PreTrainedConfig
from frrl.policies.factory import get_policy_class, make_policy_config, make_pre_post_processors
from frrl.utils.constants import ACTION
...
def main():
    ...
    # Add normalization mapping to config (convert Enums to strings)
    cleaned_config["normalization_mapping"] = {
        k.value if hasattr(k, "value") else k: v.value if hasattr(v, "value") else v
        for k, v in norm_map.items()
    }

    # Create policy configuration using draccus to handle nested dataclasses
    print(f"Creating {policy_type} policy configuration...")
    config_cls = PreTrainedConfig.get_choice_class(policy_type)
    policy_config = draccus.decode(config_cls, cleaned_config)
...
from frrl.policies import get_policy_class
