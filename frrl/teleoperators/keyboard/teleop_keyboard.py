...
from frrl.utils.errors import DeviceAlreadyConnectedError, DeviceNotConnectedError
...
class KeyboardTeleop(Teleoperator):
    ...
    def _on_press(self, key):
        if hasattr(key, "char"):
            ...
            # Also handle special keys (arrows, shift, ctrl, etc.)
...
class KeyboardEndEffectorTeleop(KeyboardTeleop):
    ...
    def __init__(self, config: KeyboardEndEffectorTeleopConfig):
        ...
        self._last_had_movement_keys = False  # Track if movement keys were pressed in last get_action()
    ...
    def get_action(self) -> dict[str, Any]:
        ...
        # WASD + Q/E 移动键，Space 夹爪
        movement_keys = ["w", "s", "a", "d", "q", "e", " ", "f"]
        self._last_had_movement_keys = any(self.current_pressed.get(key, False) for key in movement_keys)

        delta_x = 0.0
        delta_y = 0.0
        delta_z = 0.0
        gripper_action = 1.0  # 1 = 保持
        ...
        for key, val in self.current_pressed.items():
            if key == "w":
                ...
                gripper_action = 0  # Space = 关夹爪
            elif key == "f" and val:
                gripper_action = 2  # F = 开夹爪
            elif val and key not in ("w", "s", "a", "d", "q", "e", " ", "f"):
    ...
    def get_teleop_events(self) -> dict[str, Any]:
        ...
        is_intervention = self._last_had_movement_keys
...
class KeyboardRoverTeleop(KeyboardTeleop):
    ...
    Example:
        ```python
        from frrl.teleoperators.keyboard import KeyboardRoverTeleop, KeyboardRoverTeleopConfig
