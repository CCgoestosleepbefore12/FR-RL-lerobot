...
from frrl.datasets.compute_stats import aggregate_stats, compute_episode_stats
from frrl.datasets.image_writer import AsyncImageWriter, write_image
from frrl.datasets.utils import (
...
from frrl.datasets.video_utils import (
    VideoFrame,
...
from frrl.utils.constants import HF_LEROBOT_HOME
...
class LeRobotDatasetMetadata:
    ...
    def save_episode_tasks(self, tasks: list[str]):
        ...
        if self.tasks is None:
            ...
            for task_idx, task in zip(new_task_indices, new_tasks, strict=True):
    ...
    def create(
        ...
        if len(obj.video_keys) > 0 and not use_videos:
            raise ValueError(
                "Dataset features contain video keys but use_videos=False. "
                "Either remove video keys from features or set use_videos=True."
            )
...
class LeRobotDataset(torch.utils.data.Dataset):
    def __init__(
        ...
        # Initialized as None; populated lazily during dataset recording
