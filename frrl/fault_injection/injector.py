#!/usr/bin/env python3
"""EncoderBiasInjector — 每个 episode 起点采样一个 bias 向量并报告 biased qpos.

模拟编码器校准误差：bias 同时影响控制器（通过 biased Jacobian）和观测
（通过 biased FK）。由 ``EncoderBiasConfig`` 驱动，``on_episode_start`` 决定
本集是否注入故障并采样 bias 向量。
"""
