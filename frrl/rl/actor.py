...
def act_with_policy(
    ...
    for interaction_step in range(cfg.policy.online_steps):
        ...
        # InputsControlWrapper 将实际执行的动作写入 info["teleop_action"]
        executed_action = new_transition[TransitionKey.INFO].get(
            "teleop_action", new_transition[TransitionKey.ACTION]
        )

        # Ensure executed_action is a tensor with batch dimension for buffer compatibility
        if not isinstance(executed_action, torch.Tensor):
            executed_action = torch.tensor(executed_action, dtype=torch.float32)
        if executed_action.dim() == 1:
            executed_action = executed_action.unsqueeze(0)  # Add batch dim: [4] -> [1, 4]
        ...
        if done or truncated:
            ...
            # Send episodic metrics to the learner
            interaction_message = {
            ...
            last_info = new_transition.get(TransitionKey.INFO, {})
            for key in ["hand_collisions", "min_hand_dist", "displacement", "speed_scale"]:
                if key in last_info:
                    interaction_message[key] = last_info[key]
            interactions_queue.put(python_object_to_bytes(interaction_message))
