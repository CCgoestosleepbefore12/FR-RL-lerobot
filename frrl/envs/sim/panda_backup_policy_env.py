...
D_TIGHT_ARM = 0.08               # V3c arm_center tracking 的"停顿距离"——但 0.08 < collision_dist
                                  # 0.20，dwell trigger 永远进不去，**实质关闭 dwell 行为**。
                                  # V3c 重设计 (2026-04-27 cross-eval 后)：发现纯 dwell 训练
                                  # 让 policy brittle (V3c 在 V3 env 仅 65%, hand_collision 36%)。
                                  # 改回 V3-style "持续追逼 escape avoid"，但 tracking_target
                                  # 仍 arm_center（保 sim2real FSM 几何对齐）+ 加压 hand 速度
                                  # / episode 长度，让训练分布严于 V3。
...
HAND_SPEED_RANGE_V3 = (0.015, 0.030)   # m/step，对齐真机人手 15-30cm/s 区间 (V3 / V3b 用)
HAND_SPEED_RANGE_V3C = (0.020, 0.040)  # m/step，V3c 加压：v_hand max > v_arm max(0.030)，
                                        # closure rate +0.010 m/step worst-case 让训练分布严于 V3
...
class PandaBackupPolicyEnv(PandaPickPlaceSafeEnv):
    ...
    def reset(self, seed=None, **kwargs) -> Tuple[Dict[str, Any], Dict[str, Any]]:
        ...
        # 追踪速度 per-episode 随机；V3c (arm_center tracking) 加压用更快区间，
        # V3/V3b (TCP tracking + use_full_arm_collision) 用 V3 区间，V1/V2 用旧区间
        if self._tracking_target == "arm_center":
            speed_range = HAND_SPEED_RANGE_V3C
        elif self._use_full_arm_collision:
            speed_range = HAND_SPEED_RANGE_V3
        else:
            speed_range = HAND_SPEED_RANGE
