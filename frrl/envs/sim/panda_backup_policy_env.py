...
D_TIGHT_ARM = 0.23               # V3c arm_center tracking 的停顿距离。
                                  # 数学下限 = ARM_COLLISION_DIST_V3 (0.20) + hand_speed_max
                                  # (0.030) = 0.23m，hand 单步从 dwell 边界外 ε 进入到 dwell
                                  # 区域内，下一步 collision check 仍 > 0.20m，不撞。
                                  # 选 0.23 (1cm 缓冲) 而非 0.25 (5cm) 提升威胁帧密度——
                                  # hand 在 23-30cm 区域占据更多 episode 时间，policy 学到
                                  # 的"近距离 dwell"样本更多。代价：探索期 ~10% collision
                                  # (policy 反向移动 arm 时 1-step 撞)，训练稳态 < 5%。
