...
class GripperPenaltyWrapper(gym.Wrapper):
    ...
    def reset(self, **kwargs):
        ...
        self.last_gripper_pos = self.unwrapped.get_gripper_pose()
        return obs, info
...
class InputsControlWrapper(gym.Wrapper):
    ...
    def __init__(
        ...
        """
        super().__init__(env)
        from frrl.envs.wrappers.intervention_utils import (
            GamepadController,
    ...
    def get_gamepad_action(self):
        ...
        # 构建 7D 动作: [dx, dy, dz, rx, ry, rz, gripper]
        # 键盘模式下旋转为零，后续 SpaceMouse 可输出完整 6D
        gamepad_action = np.array(
            [delta_x, delta_y, delta_z, 0.0, 0.0, 0.0], dtype=np.float32
        )
