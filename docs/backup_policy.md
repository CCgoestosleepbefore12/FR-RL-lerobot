...
```python
# 终止条件（r = -1.0）
if collision:      terminated, r = -1.0   # P0: 碰到人手
if block_dropped:  terminated, r = -1.0   # P1: 方块掉落
if zone_c:         terminated, r = -1.0   # P0: 进入人员禁区

# 正常存活结束（10 步到期）
if truncated:      r = 0.0

# 存活每步
r = - 0.5  * ||tcp_current - tcp_start||     # P2: 位移惩罚（别跑远）
    - 0.01 * ||action||                      # P3: 动作幅度惩罚
    - 0.01 * ||action - action_prev||         # P3: 动作平滑惩罚
```
...
| 优先级 | 目标 | 奖励项 | 说明 |
|--------|------|--------|------|
| P0 | 不碰人手 / 不越界 | collision/zone_c → -1 | 安全底线，稀疏惩罚 |
| P1 | 不掉方块 | block_dropped → -1 | 闪避不能牺牲任务状态 |
| P2 | 原地闪避 | -0.5 × 位移 | 10 步后交还 task policy，太远回不来 |
| P3 | 动作平滑 | 幅度 + 变化量 | 真机关节力矩/加速度有限 |

**不使用 proximity reward**（距离奖励）：
- 鼓励"尽量远离"，与位移惩罚矛盾
- 导致过度反应，真实目标是"刚好躲开"
