...
验证脚本：
```bash
python scripts/test_d455.py             # 枚举 + 打开所有 D455
python scripts/test_d455.py --show      # 实时预览每路
python scripts/test_dual_camera.py      # 30s 带宽压测（FPS / drop / USB topology）
```
...
```bash
python scripts/collect_demo_task_policy.py -n 50      # 采 50 条成功 demo
python scripts/collect_demo_task_policy.py -n 5 --no-bias   # 调试用，不注入 bias
```
...
输出文件名格式：`demos/task_policy/task_policy_demos_{N}_{complete|aborted}_{YYYY-MM-DD_HH-MM-SS}.pkl`。

### 验证

```bash
python scripts/inspect_demo_pickle.py demos/task_policy/*.pkl
```
...
# 复制输出 JSON 到 configs/train_task_policy_franka.json 的 policy.dataset_stats 下
...
**主配置**：`configs/train_task_policy_franka.json`
...
A: 跑 `scripts/compute_dataset_stats.py --demos "demos/task_policy/*.pkl"` 自动从真实 demo 算。也可以保留 config 里现有的理论边界（Franka 关节极限、workspace bounding box 等）。
