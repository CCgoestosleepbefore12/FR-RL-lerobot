...
- ✅ **V3c r3 单参数 ablation (2026-04-27)**：经历 r1 (dwell-based 训 brittle, 65% on V3 env) + r2 (加压训不收敛, 62% own env) 两次失败后，r3 回归最小变更：和 V3b 完全一样，**唯一**差异是 `tracking_target='arm_center'`（hand 追 panda_hand body = collision 球心 = flange）而非 TCP。300k 同预算训练，cross-eval 后比 V3b 看 tracking_target 选择对 policy 影响。env / 注册 / 测试 (27/27 pass) / 文档同步完成
- ⏳ V3c r3 训练 300k 待启动（port 50056, seed 1005, utd=4, ~6.3h）
