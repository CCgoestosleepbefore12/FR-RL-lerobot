...
- ⏳ Path A 重训：`max_displacement` 0.40 → 0.50（针对 14.5% excessive_displacement 失效）
...
| `checkpoints/backup_policy_s1_v3_300k_71pct/` | V3 全臂避障首训（max_disp=0.40, 71.5% 存活率）| 部署用 `--ckpt-version v3` (D_SAFE=0.40/D_CLEAR=0.45) |
| `checkpoints/backup_policy_s1_v3b_*` | V3 Path A 重训（max_disp=0.50, 待训完）| 同上 |
