...
- ✅ **V3c 重设计 (2026-04-27 cross-eval 后)**：V3c 早版 (dwell-based, D_TIGHT_ARM=0.23) 190k cross-eval 揭示分布外 brittle——V3c policy 在 V3 env 仅 65% (hand_collision 36.5%)，V3b 反而在两个 env 都 ≥ 95%。dwell 训练分布太 benign 让 policy 没学到应对持续追逼。重设计：保留 arm_center tracking (sim2real FSM 几何对齐) 但**改训练分布严于 V3** —— D_TIGHT_ARM 0.23→0.08 关 dwell + HAND_SPEED_RANGE_V3C (0.020,0.040) 加压 + episode 20→25 步累积 closure。env / 注册 / 测试 (27/27 pass) / 文档同步完成
- ⏳ V3c 重训 400k 待启动（port 50056, seed 1005, utd=4；早版 190k ckpt 弃用）
