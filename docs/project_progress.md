...
- ✅ **V3c 设计 + 实施 + P1 修复 (2026-04-27)**：发现 V3 D_TIGHT=8cm 实质 unreachable kill zone（hand 在 D_TIGHT 几何已 < collision_dist），改 `tracking_target=arm_center` + `D_TIGHT_ARM=23cm`（数学下限 = collision_dist + hand_speed_max）让 dwell 行为成为可达样本，更贴近真机"人手伸近停下观察"语义。env / 注册 / config / shell / 测试 (26/26 pass) / 文档全部就位；P1 修复包括 D_TIGHT_ARM 0.25→0.23、测试 placement 鉴别力、多球反向 dwell 验证、cross-eval 协议与 fast-approach 测试落档
- ⏳ V3c 训练 400k 待启动（port 50056, seed 1005, utd=4，~8h；V3c stall 让威胁帧密度降 30-50%，从 V3b 300k 加到 400k 留足训练预算）
