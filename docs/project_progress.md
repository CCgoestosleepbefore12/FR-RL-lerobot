...
- ✅ Eval ckpt 落库 `checkpoints/backup_policy_s1_v3_300k_71pct/`（max_disp=0.40, ablation 对照）
...
- ✅ Final ckpt 落库 `checkpoints/backup_policy_s1_v3b_300k_95pct/` ⭐ **真机首选**
- ✅ 中段 ckpt 落库 `checkpoints/backup_policy_s1_v3b_115k_90pct/`（训练曲线记录）
- ✅ **V3c r3 单参数 ablation 完成 (2026-04-27/28)**：经历 r1 (dwell-based 训 brittle) + r2 (加压训不收敛) 两次失败后，r3 回归最小变更（仅 tracking_target='arm_center'）。300k 训完 cross-eval：V3 env **96.5%** (反超 V3b 95%) / V3c env 93.5% / **min 93.5%**（vs V3b min 95%，−1.5%）。落库 `checkpoints/backup_policy_s1_v3c_r3_300k_94pct/`
...
| `checkpoints/backup_policy_s1_v3c_r3_300k_94pct/` | V3c r3 单参数 ablation (tracking_target=arm_center, 其它同 V3b)，min cross-eval 93.5% (V3 env 96.5% / V3c env 93.5%) | `--ckpt-version v3`（与 V3b 共用 D_SAFE/D_CLEAR；论文 ablation 对照，性能 −1.5% 略弱于 V3b）|
