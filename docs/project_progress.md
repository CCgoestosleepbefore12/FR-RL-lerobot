...
四种运动模式（ARC / LINEAR / PASSING / STOP_GO）全部 ≥ 97.8%。Checkpoint：`checkpoints/backup_policy/backup_policy_s1/`。
...
Checkpoints：
- `checkpoints/backup_policy/backup_policy_s1_v2_50k` — V2 50k 训练版
- `checkpoints/backup_policy/backup_policy_s1_v2_newgeom_35k` — 真机实验首选（小 ckpt）
- `checkpoints/backup_policy/backup_policy_s1_v2_newgeom_145k` — 训练峰值，真机备用
...
实施状态（2026-04-26 / 04-27）：
- ✅ env / 注册 / config / 启动脚本 / 单测 (21/21 pass) / FSM (CLI `--ckpt-version` 自动配阈值) / 文档
- ✅ 首训 300k 完成（6.5h）→ **71.5% 满存活率**（远低于 meta 预测 92-95%，hand_collision 17.5% / excessive_displacement 14.5%）
- ✅ Eval ckpt 落库 `checkpoints/backup_policy/backup_policy_s1_v3_300k_71pct/`（max_disp=0.40, ablation 对照）
- ✅ Path A 重训完成（max_disp 0.40 → 0.50, 300k, 6.3h）→ **95.0% 满存活率**（+23.5%！）。hand_collision **17.5% → 3.0%**（multi-sphere 避让完全学到），excessive_displacement 14.5% → 5.5%
- ✅ Final ckpt 落库 `checkpoints/backup_policy/backup_policy_s1_v3b_300k_95pct/` ⭐ **真机首选**
- ✅ 中段 ckpt 落库 `checkpoints/backup_policy/backup_policy_s1_v3b_115k_90pct/`（训练曲线记录）
- ✅ **V3c r3 单参数 ablation 完成 (2026-04-27/28)**：经历 r1 (dwell-based 训 brittle) + r2 (加压训不收敛) 两次失败后，r3 回归最小变更（仅 tracking_target='arm_center'）。300k 训完 cross-eval：V3 env **96.5%** (反超 V3b 95%) / V3c env 93.5% / **min 93.5%**（vs V3b min 95%，−1.5%）。落库 `checkpoints/backup_policy/backup_policy_s1_v3c_r3_300k_94pct/`
- ✅ **结论：V3b 真机首选不变**。V3c r3 论文 ablation 价值：tracking_target 选择对 policy 质量影响微小 (~1-2%)，sim 训练 target 不必跟真机 FSM 反推点 1:1 对齐；训练分布的 "balanced harshness" 比 sim2real 几何对齐更重要 (r1 brittle / r2 不收敛 / r3 接近)

| ckpt | sim 存活率 | hand_collision | 真机肘撞 | 部署 |
|---|---|---|---|---|
| V2 145k (单球) | 100% | <1% | **存在** | `--ckpt-version v2` (D_SAFE=0.30) |
| V3 300k (max_disp=0.40) | 71.5% | 17.5% | 根除 | ablation 对照 |
| **V3b 300k (max_disp=0.50)** | **95.0%** | **3.0%** | **根除** | `--ckpt-version v3` (D_SAFE=0.40) ⭐ |
...
| 目录 | 用途 | 状态 |
|------|------|------|
| `checkpoints/backup_policy/backup_policy_s1/` | Exp 3 Backup S1 DR，200k | 99.0% |
| `checkpoints/backup_policy/backup_policy_s1_v2_50k/` | V2 几何 50k | 训练中期 |
| `checkpoints/backup_policy/backup_policy_s1_v2_newgeom_35k/` | V2 新几何 | 部署用 `--ckpt-version v2` (D_SAFE=0.30/D_CLEAR=0.35) |
| `checkpoints/backup_policy/backup_policy_s1_v2_newgeom_145k/` | V2 新几何训练峰值 | 部署用 `--ckpt-version v2` (D_SAFE=0.30/D_CLEAR=0.35) |
| `checkpoints/backup_policy_s2/` | Exp 7 Backup S2 多障碍，200k | 83-91% |
| `checkpoints/backup_policy/backup_policy_s1_v3_300k_71pct/` | V3 首训（max_disp=0.40, 71.5%, ablation 对照） | `--ckpt-version v3` |
| `checkpoints/backup_policy/backup_policy_s1_v3b_115k_90pct/` | V3 Path A 中段（max_disp=0.50, 115k, 89.5%, 训练曲线记录）| `--ckpt-version v3` |
| `checkpoints/backup_policy/backup_policy_s1_v3b_300k_95pct/` | **V3 Path A 训完版（max_disp=0.50, 300k, 95%, hand_collision 3%）** ⭐ | **真机首选**，`--ckpt-version v3` |
| `checkpoints/backup_policy/backup_policy_s1_v3c_r3_300k_94pct/` | V3c r3 单参数 ablation (tracking_target=arm_center, 其它同 V3b)，min cross-eval 93.5% (V3 env 96.5% / V3c env 93.5%) | `--ckpt-version v3`（与 V3b 共用 D_SAFE/D_CLEAR；论文 ablation 对照，性能 −1.5% 略弱于 V3b）|
