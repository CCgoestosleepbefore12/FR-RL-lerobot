...
- ✅ **V3c 设计 + 实施 (2026-04-27)**：发现 V3 D_TIGHT=8cm 实质 unreachable kill zone（hand 在 D_TIGHT 几何已 < collision_dist），改 `tracking_target=arm_center` + `D_TIGHT_ARM=25cm` 让 dwell 行为成为可达样本，更贴近真机"人手伸近停下观察"语义。env / 注册 / config / shell / 测试 (25/25 pass) / 文档全部就位
- ⏳ V3c 训练 300k 待启动（port 50056, seed 1005）
