...
| `~/FR-RL-lerobot/frrl/envs/` | `real.py` / `real_config.py` | 无改动（hook 已有；Stage 2 重命名前为 `franka_real_env.py` / `franka_real_config.py`） | ✅ git |
| `~/FR-RL-lerobot/frrl/` | `fault_injection.py` | 无改动（`EncoderBiasInjector` / `EncoderBiasConfig`） | ✅ git |
...
`frrl/envs/real.py::FrankaRealEnv` 的 reset 注入 hook 和 `_set_encoder_bias`
方法在本轮工作之前就已存在（Stage 2 重构前文件名为 `franka_real_env.py`，line
number 已变动，以符号名为准），**路由名和 JSON 格式完全吻合**新增的 franka_server
接口。GPU 侧仅需在构造 `FrankaRealConfig` 时：
...
`EncoderBiasConfig.from_yaml` 已实现，**仓库当前没有 bias YAML 预设**
（需要时自建，真机推荐放到 `scripts/configs/` 目录下和训练 JSON 预设同组织）。
示例：
```yaml
# scripts/configs/bias_j1_random.yaml  (示例，按需创建)
enable: true
error_probability: 1.0
target_joints: [0]
bias_mode: random_uniform
bias_range: [-0.15, 0.15]   # 真机 J1 常用范围
```
```python
from frrl.fault_injection import EncoderBiasConfig
bias_cfg = EncoderBiasConfig.from_yaml("scripts/configs/bias_j1_random.yaml")
```
